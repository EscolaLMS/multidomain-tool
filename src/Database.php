...
class Database
{
    public static function createPostgresUserAndDatabase($domain)
    {
        ...
        $query = "ALTER DATABASE {$username} OWNER TO {$username};";
        pg_prepare($dbconn, "", $query);
        $result = pg_execute($dbconn, "", []);

        if ($result) {
            echo $color->ok("Postgres. Owner granted to $username on database $username \n");
        } else {
            echo $color->warn("Postgres. Owner granted to $username Failed \n");
            echo $color->error(pg_last_error() . "\n");
            return [
                'success' => false
            ];
